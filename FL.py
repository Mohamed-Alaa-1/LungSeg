...
def main():
    ...
    try:
        ...
        # =====================================================================================
        # STEP 1: Creating Global Train/Val/Test Splits (UNCHANGED)
        # =====================================================================================
        logger.info("\n" + "="*80)
        ...
        # =====================================================================================
        # STEP 2: Creating Virtual Clients from Training Data (UNCHANGED)
        # =====================================================================================
        logger.info("\n" + "="*80)
        ...
        max_client_size = int(np.median(train_sizes))
        ...
        # =====================================================================================
        # STEP 3: CREATE CLIENT DATA LOADERS - FIXED VERSION
        # =====================================================================================
        logger.info("\n" + "="*80)
        logger.info("STEP 3: Creating Client Data Loaders (FIXED - No Overlap)")
        logger.info("="*80)
        ...
        # 🔧 FIX: Track offsets to prevent overlap
        dataset_val_offset = {}
        dataset_test_offset = {}

        for dataset_name, _ in all_datasets:
            dataset_val_offset[dataset_name] = 0
            dataset_test_offset[dataset_name] = 0

        logger.info("\n📊 Val/Test Distribution Plan:")

        # First pass: Calculate sizes to ensure we have enough data
        dataset_val_required = {}
        dataset_test_required = {}

        for client_name, train_data, source_dataset in virtual_clients:
            # Calculate proportion for this client
            total_train_in_dataset = sum(
                len(data) for name, data, src in virtual_clients if src == source_dataset
            )
            ...
            if source_dataset not in dataset_val_required:
                dataset_val_required[source_dataset] = 0
                dataset_test_required[source_dataset] = 0
            
            dataset_val_required[source_dataset] += val_size
            dataset_test_required[source_dataset] += test_size

        # Check if we have enough data
        for dataset_name in dataset_val_required:
            val_available = len(dataset_val_data[dataset_name])
            test_available = len(dataset_test_data[dataset_name])
            val_needed = dataset_val_required[dataset_name]
            test_needed = dataset_test_required[dataset_name]
            
            logger.info(f"\n{dataset_name}:")
            logger.info(f"  Val:  {val_needed} needed, {val_available} available")
            logger.info(f"  Test: {test_needed} needed, {test_available} available")
            
            if val_needed > val_available:
                logger.warning(f"  ⚠️  Val shortage: {val_needed - val_available} samples")
            if test_needed > test_available:
                logger.warning(f"  ⚠️  Test shortage: {test_needed - test_available} samples")

        # Second pass: Actually assign data with offset tracking
        ...
        for client_name, train_data, source_dataset in virtual_clients:
            ...
            # 🔧 FIX: Assign NON-OVERLAPPING validation samples
            val_size = max(1, int(len(source_val_data) * proportion))
            val_start = dataset_val_offset[source_dataset]
            val_end = min(val_start + val_size, len(source_val_data))
            
            # Ensure we don't go out of bounds
            if val_start >= len(source_val_data):
                logger.warning(f"  ⚠️  {client_name}: No more val data available, using last sample")
                client_val_subset = [source_val_data[-1]]
            else:
                client_val_subset = source_val_data[val_start:val_end]
            
            dataset_val_offset[source_dataset] = val_end
            
            # 🔧 FIX: Assign NON-OVERLAPPING test samples
            test_size = max(1, int(len(source_test_data) * proportion))
            test_start = dataset_test_offset[source_dataset]
            test_end = min(test_start + test_size, len(source_test_data))
            
            # Ensure we don't go out of bounds
            if test_start >= len(source_test_data):
                logger.warning(f"  ⚠️  {client_name}: No more test data available, using last sample")
                client_test_subset = [source_test_data[-1]]
            else:
                client_test_subset = source_test_data[test_start:test_end]
            
            dataset_test_offset[source_dataset] = test_end
            ...
            logger.info(f"\n{client_name} (from {source_dataset}):")
            logger.info(f"  Train: {len(train_data)} samples")
            logger.info(f"  Val:   {len(client_val_subset)} samples (indices {val_start}:{val_end})")
            logger.info(f"  Test:  {len(client_test_subset)} samples (indices {test_start}:{test_end})")

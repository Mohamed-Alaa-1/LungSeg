    ...
    def forward(self, inputs)
        ...
        for i, layer in enumerate(self.trans_encoder_layers)
            ...
            if torch.isnan(x).any()
                raise ValueError(fNaN detected after transformer layer {i})
    ...
    for name, param in model.named_parameters()
        if param.grad is not None
            ...
            if torch.isnan(param.grad).any()
                logger.error(fNaN gradient detected in {name})
...
def train_one_fold(fold, train_files, val_files, test_files, model_config, config)
    ...
    fold_logger.info(=60)
    fold_logger.info(fSTARTING FOLD {fold + 1}3)
    fold_logger.info(=60)
    fold_logger.info(fTraining samples {len(train_files)})
    fold_logger.info(fValidation samples {len(val_files)})
    fold_logger.info(fTesting samples {len(test_files)})
    ...
    # Training loop
    fold_logger.info(fStarting training for Fold {fold + 1})
    ...
    for epoch in range(config.MAX_EPOCHS)
        ...
        epoch_loss = 0
        batch_count = 0
        progress_bar = tqdm(train_loader, desc=fFold {fold+1} - Epoch {epoch + 1}{config.MAX_EPOCHS}, unit=batch)
            ...
            try
                ...
                with autocast()
                    try
                        ...
                        if NaN in str(e)
                            fold_logger.warning(fNaN detected during forward pass. Skipping batch.)
                            continue
                        else
                            raise
                ...
                if out of memory in str(e)
                    ...
                    fold_logger.error(fRuntime error {e})
                    raise
        ...
        with torch.no_grad()
            ...
            for val_data in val_loader
                try
                    ...
                    fold_logger.warning(fError during validation {e})
                    continue
        ...
        fold_logger.info(fFold {fold+1} - Epoch {epoch + 1} - Train Loss {avg_epoch_loss.4f}  Val Loss {avg_val_loss.4f}  Dice {metric.4f}  Acc {accuracy.4f}  LR {current_lr.2e})

        # Stability checks
        if avg_epoch_loss  config.LOSS_EXPLOSION_THRESHOLD
            consecutive_bad_epochs += 1
            fold_logger.warning(fHigh loss detected. Bad epochs {consecutive_bad_epochs})
        ...
        if metric  best_metric
            ...
            try
                ...
                fold_logger.info(fNew best model saved! Dice {best_metric.4f} at epoch {best_metric_epoch})
            except Exception as e
                fold_logger.error(fFailed to save best model {e})
        else
            patience_counter += 1

        # Early stopping
        if patience_counter = config.PATIENCE
            fold_logger.info(fEarly stopping at epoch {epoch + 1}. No improvement for {config.PATIENCE} epochs.)
            break

    fold_logger.info(fFold {fold + 1} training finished. Best Dice {best_metric.4f} at epoch {best_metric_epoch})

    # Test on this fold's test set
    fold_logger.info(fEvaluating Fold {fold + 1} on test set...)
    ...
    with torch.no_grad()
        for test_data in tqdm(test_loader, desc=fTesting Fold {fold+1})
            try
                ...
                fold_logger.warning(fError during testing {e})
                continue
    ...
    fold_logger.info(=60)
    fold_logger.info(fFOLD {fold + 1} TEST METRICS)
    fold_logger.info(=60)
    fold_logger.info(fDice Score {mean_dice_test.4f})
    fold_logger.info(fIoU {iou_test.4f})
    fold_logger.info(fSensitivity {sensitivity.4f})
    fold_logger.info(fSpecificity {specificity.4f})
    fold_logger.info(fAccuracy {accuracy.4f})
    fold_logger.info(=60)
...
def main():
    try:
        ...
        logger.info(=60)
        logger.info(fLIGHTWEIGHT AURAVIT - {config.N_FOLDS}-FOLD CROSS VALIDATION)
        logger.info(=60)
        logger.info(fUsing device {config.DEVICE})
        logger.info(fModel configuration {MODEL_SIZE})
        logger.info(fImage size {model_config['image_size']})
        logger.info(fLayers {model_config['num_layers']})
        logger.info(fHidden dim {model_config['hidden_dim']})
        logger.info(fBatch size {config.BATCH_SIZE})
        logger.info(fNumber of folds {config.N_FOLDS})
        logger.info(fSplit ratios - Train {config.TRAIN_RATIO.0%}, Val {config.VAL_RATIO.0%}, Test {config.TEST_RATIO.0%})
        ...
        logger.info(fTotal samples {len(data_dicts)})
        ...
        for fold in range(config.N_FOLDS)
            logger.info(=60)
            logger.info(fSTARTING FOLD {fold + 1}{config.N_FOLDS})
            logger.info(=60)
            
            # Shuffle data with different seed for each fold
            np.random.seed(42 + fold)
            shuffled_indices = np.random.permutation(len(data_dicts))
            shuffled_data = [data_dicts[i] for i in shuffled_indices]
            ...
            train_files = shuffled_data[n_train]
            val_files = shuffled_data[n_trainn_train + n_val]
            test_files = shuffled_data[n_train + n_val]
            
            logger.info(fFold {fold + 1} - Train {len(train_files)}, Val {len(val_files)}, Test {len(test_files)})
            logger.info(fFold {fold + 1} - Actual ratios - Train {len(train_files)n_total.2%}, Val {len(val_files)n_total.2%}, Test {len(test_files)n_total.2%})
        ...
        logger.info(nPer-Fold Results)
        for i, result in enumerate(all_fold_results)
            logger.info(fnFold {i+1})
        ...
        logger.info(n + =60)
        logger.info(fAVERAGE METRICS ACROSS {config.N_FOLDS} FOLDS)
        logger.info(=60)
        logger.info(fMean Dice Score {np.mean(test_dice_scores).4f} ± {np.std(test_dice_scores).4f})
        logger.info(fMean IoU {np.mean(test_iou_scores).4f} ± {np.std(test_iou_scores).4f})
        logger.info(fMean Sensitivity {np.mean(test_sensitivities).4f} ± {np.std(test_sensitivities).4f})
        logger.info(fMean Specificity {np.mean(test_specificities).4f} ± {np.std(test_specificities).4f})
        logger.info(fMean Accuracy {np.mean(test_accuracies).4f} ± {np.std(test_accuracies).4f})
        logger.info(=60)
        ...
        for fold_idx, result in enumerate(all_fold_results)
            ...
            axes[fold_idx, 0].set_title(fFold {fold_idx+1} - Training Loss)
            ...
            axes[fold_idx, 1].set_title(fFold {fold_idx+1} - Validation Dice)
            ...
            axes[fold_idx, 2].set_title(fFold {fold_idx+1} - Validation Accuracy)
        ...
        plt.tight_layout()
        plt.savefig(flightweight_{MODEL_SIZE}_{n_folds}fold_cv_training_curves.png, dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.tight_layout()
        plt.savefig(flightweight_{MODEL_SIZE}_{config.N_FOLDS}fold_cv_metrics_comparison.png, dpi=300, bbox_inches='tight')
        plt.show()
        
        logger.info(f{config.N_FOLDS}-Fold Cross Validation completed successfully!)
        
    except Exception as e
        logger.error(fTraining failed with error {str(e)})
        import traceback

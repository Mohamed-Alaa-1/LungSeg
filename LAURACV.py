...
def train_one_fold(fold, train_files, val_files, test_files, model_config, config):
    ...
    fold_logger.info("="*60)
    fold_logger.info(f"STARTING FOLD {fold + 1}/3")
    fold_logger.info("="*60)
...
def main():
    try:
        ...
        for fold in range(config.N_FOLDS):
            ...
            val_files = shuffled_data[n_train:n_train + n_val]
            test_files = shuffled_data[n_train + n_val:]
            
            logger.info(f"Fold {fold + 1} - Train: {len(train_files)}, Val: {len(val_files)}, Test: {len(test_files)}")
            logger.info(f"Fold {fold + 1} - Actual ratios - Train: {len(train_files)/n_total:.2%}, Val: {len(val_files)/n_total:.2%}, Test: {len(test_files)/n_total:.2%}")
